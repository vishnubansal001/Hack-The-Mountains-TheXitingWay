...
exports.postDevlanceMainDev = async (req, res) => {
  ...
  if (token) {
    ...
    const the_post = await post.create({
...
exports.postDevVerify = async (req, res) => {
  ...
  if (token) {
    ...
    try {
      ...
      await devlancer.findOneAndUpdate(
        { _id: decoded._id },
        { $set: { git_profile: git_profile, github_verified: true } }
      );
...
exports.getProfile = async (req, res) => {
  ...
  if (token) {
    ...
    const the_user = await devlancer.findOne({
